...
{
	return value == 0.0 || value == 1.0 || value == 1.5 || value == 2.0 || value == 2.5 || value == 3.0 || value == 3.5 || value == 4.0;
}
...
float convertPercent(int num, int n)
{
	return num * 1.0 / n * 100;
}
...
main()
{
   ...
   printf("\nNumber of 4.0: %d", count4(aGrades, nElem));
   
   res = countFail(aGrades, nElem);
   
   printf("\nNumber of Failing: %d", res);
   
   printf("\nPercentage of Failing: %.2f%%", convertPercent(res, nElem));
   
   res = countPass(aGrades, nElem);
   
   printf("\nNumber of Passing: %d", res);

   printf("\nPercentage of Passing: %.2f%%", convertPercent(res, nElem));
